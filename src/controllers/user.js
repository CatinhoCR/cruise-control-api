...
async function saveUser(req, res) {
  ...
  User.findOne({
    ...
    if (err)
      return res.status(500).send({ message: 'Error en la petición al server' })
    if (users) {
      return res.status(200).send({
        message: 'El usuario ya existe',
      })
    } else {
      // If user doesnt exist encrypt pw
      if (params.password) {
        const saltRounds = 10
            ...
            user.save((err, userStored) => {
              if (err) {
                ...
                if (!userStored) {
                  res
                    .status(404)
                    .send({ message: 'No se ha registrado pues..' })
                } else {
                  res.status(200).send({
      ...
      if (!user) {
        ...
        bcrypt.compare(password, user.password, function (err, check) {
          if (check) {
            ...
            const jwtBearerToken = jwt.createToken(user)
            if (params.getHash) {
              // res.status(200).json({
